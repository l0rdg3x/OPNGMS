...
async def _seed_multi_label(db_engine, tenant_id, device_id):
    """Inserts 2 timestamps for each of 2 labels (igb0, igb1)."""
    factory = async_sessionmaker(db_engine, expire_on_commit=False)
    base = datetime(2026, 6, 9, 12, 0, tzinfo=timezone.utc)
    rows = [
        ("igb0", 0, 10.0),
        ("igb0", 1, 11.0),  # last for igb0
        ("igb1", 0, 20.0),
        ("igb1", 1, 21.0),  # last for igb1
    ]
    async with factory() as s:  # owner -> bypasses RLS
...
async def test_last_returns_latest_per_distinct_label(db_engine, two_tenants):
    ...
    # Sort by label for stability: last value of EACH label (DISTINCT ON).
...
async def test_series_and_last_empty_when_no_data(db_engine, two_tenants):
    tenant_a, _ = two_tenants
    device_id = uuid.uuid4()  # no data for this device
    factory = async_sessionmaker(db_engine, expire_on_commit=False)
    async with factory() as s:
        await s.execute(text(f"SET ROLE {APP_ROLE}"))
        await set_tenant_context(s, tenant_a)
...
async def _seed_four(db_engine, tenant_id, device_id):
    """4 cpu.load metrics (label '') at increasing minutes with values 10,20,30,40."""
    factory = async_sessionmaker(db_engine, expire_on_commit=False)
    base = datetime(2026, 6, 9, 12, 0, tzinfo=timezone.utc)
    async with factory() as s:  # owner -> bypasses RLS
...
async def test_series_truncates_to_most_recent_points(db_engine, two_tenants, monkeypatch):
    # With MAX_POINTS=2 and 4 points (10,20,30,40), the raw series must return the 2 MOST
    # RECENT (30,40) in ascending order, not the 2 oldest (10,20).
