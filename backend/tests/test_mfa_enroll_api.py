...
async def _seed_user(db_engine, email="u@x.io", password="pw12345"):
...
async def _login(api_client, email="u@x.io", password="pw12345"):
...
async def test_disable_requires_password_and_clears_mfa(api_client, db_engine):
    ...
    rok = await api_client.post("/api/me/mfa/disable", json={"password": "pw12345"}, headers=h)
...
async def test_regenerate_recovery_codes(api_client, db_engine):
    ...
    rgen = await api_client.post(
        "/api/me/mfa/recovery/regenerate", json={"password": "pw12345"}, headers=h
    )
