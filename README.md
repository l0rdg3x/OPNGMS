...
> + capability discovery — 4C — firewall-aware config UI — and 4D-a — change/push pipeline (dry-run) —
...
| **4 — Config management** | Versioned, encrypted config backup + drift detection (schema-agnostic, RLS). 4A backup+drift ✅ · 4B config model + capability ✅ · 4C firewall-aware UI ✅ · 4D edit + push (4D-a pipeline ✅, dry-run) | 🔄 In progress (4A–4C, 4D-a ✅) |
